...
class ConversationalChainWrapper:
    ...
    def init_LLM(self):
        ...
        #     repo_id=self.repo_id, max_length=200, temperature=0.1, token=self.token
        # )
...
if __name__ == "__main__":
    ...
    context_metadata_filename = "2016_01_french_with_metadata.csv"

    conversational_chain = ConversationalChainWrapper(repo_id, huggingfacehub_api_token, context_metadata_filename)

    json_data = {'question': [], 'ground_truth': []}
    with open('dataset/french_data_qa.jsonl') as f:
        for line in f.readlines():
            dic = json.loads(line)
            json_data['question'].append(dic['question'])
            json_data['ground_truth'].append(dic['answer'])

    dataset = QuestionsDataset(json_data)
    data_loader = DataLoader(dataset, batch_size=8, shuffle=False)

    name = "Phi-3-mini-4k-instruct-french"
    ...
    for i, batch in enumerate(data_loader):
        try:
            ...
            with open(f"out/{name}-{time}.json", 'w') as f:
